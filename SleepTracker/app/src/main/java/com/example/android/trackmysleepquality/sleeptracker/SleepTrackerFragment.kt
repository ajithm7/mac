...
class SleepTrackerFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val dataSourse=SleepDatabase.getInstance(appliction).sleepDatabaseDao
        val viewModelFactory=SleepTrackerViewModelFactory(dataSourse,appliction)
        ...
        sleepTrackerViewModel.nights.observe(viewLifecycleOwner,Observer{
            it?.let{
                val adapter=SleepNightAdapter(requireContext(),it)
                binding.sleepList.adapter=adapter
                binding.sleepList.layoutManager=LinearLayoutManager(requireContext())
            }
        })
